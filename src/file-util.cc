...
void FileUtil::DuplicateDescriptor(int new_fd, int old_fd) {
    if (dup2(new_fd, old_fd) != old_fd) {
        throw FileException("Unable to duplicate descriptor " + to_string(new_fd));
    }
}
...
vector<string> FileUtil::GetGlobMatches(const string& glob_pattern) {
    ...
    for (size_t i = 0; i < pattern_segments.size(); i++) {
        ...
        // Ensure that repeated path separators (i.e '//') are ignored,
        // except when they appear after a path segment that ends in an alphanumeric character.
...
bool FileUtil::IsFile(const string& path) {
    ...
    return stat_result.st_mode & S_IFREG;
}